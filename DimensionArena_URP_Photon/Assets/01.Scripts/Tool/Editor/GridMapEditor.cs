...
namespace GRITTY
{
    public class GridMapEditor : EditorWindow
    {
        ...
        private static void OpenMap()
        {
            ...
            //Texture Setting
            BaseGroundTexture.MakeBrownTexture();
            BaseGroundTexture.MakeDarkBrownTexture();
        }
        ...
        void RestoreGridMap()
        {
            GameObject parentObstacle = GameObject.FindWithTag("ParentObstacle");

            if (parentObstacle)
            {
                //오브젝트가 존재한다면,,
                if (parentObstacle.transform.childCount > 0)
                {
                    for (int i = 0; i < parentObstacle.transform.childCount; ++i)
                    {
                        ParsingToNode parsingData = parentObstacle.transform.GetChild(i).GetComponent<ParsingToNode>();
                        if (parsingData)
                        {
                            ...
                            list_Node[parsingData.idx.y][parsingData.idx.x].block =
                                parentObstacle.transform.GetChild(i).gameObject;
        ...
        {
            if (row < 0 || row > list_Node.Count || colmn < 0 || colmn > list_Node[row].Count)
                return;
